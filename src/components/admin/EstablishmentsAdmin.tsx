...
function ImportModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-md grid place-items-center p-4" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="max-w-2xl w-full bg-zinc-900 border border-zinc-800 rounded-3xl p-6">
        ...
        <p className="text-xs text-zinc-400 mb-3">Format attendu : <code>name,city,country,address,phone,website,email,description,categories</code></p>

        <textarea value={csv} onChange={(e) => setCsv(e.target.value)} rows={10} placeholder="Colle ton CSV ici…" className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-xs font-mono" />

        {result && (
          <div className="mt-3 p-3 bg-emerald-500/10 border border-emerald-500/30 rounded-lg text-xs text-emerald-200">
            {result.ok ? `✓ ${result.created} établissements créés${result.skipped ? ` · ${result.skipped} ignorés (doublons)` : ''}` : `❌ ${result.error}`}
          </div>
        )}

        <div className="flex gap-2 mt-4">
          <button onClick={onClose} className="flex-1 bg-zinc-800 hover:bg-zinc-700 px-4 py-2 rounded-lg text-sm">Fermer</button>
          <button onClick={importNow} disabled={!csv.trim() || busy} className="flex-1 bg-gradient-to-r from-cyan-500 to-emerald-500 text-white font-bold px-4 py-2 rounded-lg text-sm flex items-center justify-center gap-2 disabled:opacity-50">
            {busy ? <Loader2 size={14} className="animate-spin" /> : <Upload size={14} />} Importer
          </button>
        </div>
      </div>
    </div>
