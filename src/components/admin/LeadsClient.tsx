...
import {
  ...
  Tag, CheckCircle2, X, Trash2, AlertTriangle, Filter, Download, Users
} from 'lucide-react';
...
export function LeadsClient() {
  ...
  async function load() {
    ...
    const r = await fetch(`/api/admin/leads?${params}`, { cache: 'no-store' });
    ...
    <div className="px-3 lg:px-4 pb-6 max-w-7xl mx-auto">
      ...
      <section className="bg-zinc-900 ring-1 ring-zinc-800 rounded-2xl p-3 mb-4">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-xs">
            <option value="">Tous statuts</option>
            {Object.entries(STATUS_META).map(([id, m]) => <option key={id} value={id}>{m.label}</option>)}
          </select>
          <select value={filterSource} onChange={(e) => setFilterSource(e.target.value)} className="bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-xs">
      ...
      {loading ? (
        ...
        <div className="bg-zinc-900 ring-1 ring-zinc-800 rounded-2xl p-12 text-center">
          ...
          <p className="text-xs text-zinc-500">Importe un CSV ou crée-en un manuellement.</p>
        </div>
      ) : (
                    ...
                    <tr key={l.id} className="hover:bg-zinc-800/30">
                      ...
                      <td className="p-3">
                        <div className="flex gap-1">
                          {l.linkedinUrl && <a href={l.linkedinUrl} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300"><Linkedin size={11} /></a>}
                          {l.twitterUrl && <a href={l.twitterUrl} target="_blank" rel="noopener noreferrer" className="text-sky-400 hover:text-sky-300"><Twitter size={11} /></a>}
                          {l.instagramUrl && <a href={l.instagramUrl} target="_blank" rel="noopener noreferrer" className="text-pink-400 hover:text-pink-300"><Instagram size={11} /></a>}
                        </div>
                      </td>
                      <td className="p-3">
      ...
      {showImport && <ImportCSVModal onClose={() => setShowImport(false)} onDone={() => { setShowImport(false); load(); }} />}
      {showCreate && <CreateLeadModal onClose={() => setShowCreate(false)} onDone={() => { setShowCreate(false); load(); }} />}
    </div>
  );
}
...
function ImportCSVModal({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
  ...
  async function importCSV() {
    ...
    try {
      ...
      const r = await fetch('/api/admin/leads/import', { method: 'POST', body: fd });
      const j = await r.json();
    ...
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-zinc-900 ring-1 ring-zinc-800 rounded-2xl max-w-lg w-full p-5 space-y-3" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-2">
          <h3 className="font-bold flex items-center gap-2"><UploadCloud size={16} /> Import CSV</h3>
        ...
        {!result ? (
          <>
            ...
            <div>
              ...
              <input value={segments} onChange={(e) => setSegments(e.target.value)} placeholder="press, partenaires, beta-testeurs…" className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-xs" />
            </div>
            <label className="flex items-center gap-2 text-xs text-zinc-300 cursor-pointer">
              <input type="checkbox" checked={optIn} onChange={(e) => setOptIn(e.target.checked)} className="accent-emerald-500" />
              <span>Marquer comme opt-in newsletter (⚠ uniquement si le CSV vient d'un consentement vérifié — RGPD)</span>
            </label>
...
function CreateLeadModal({ onClose, onDone }: { onClose: () => void; onDone: () => void }) {
    ...
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-zinc-900 ring-1 ring-zinc-800 rounded-2xl max-w-lg w-full p-5" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-3">
          <h3 className="font-bold">Nouveau lead</h3>
          <button onClick={onClose}><X size={18} className="text-zinc-400 hover:text-white" /></button>
        </div>
        <div className="grid grid-cols-2 gap-2">
          <input placeholder="Prénom" onChange={(e) => setForm({ ...form, firstName: e.target.value })} className="bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-xs" />
          <input placeholder="Nom" onChange={(e) => setForm({ ...form, lastName: e.target.value })} className="bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-xs" />
          <input placeholder="Email" onChange={(e) => setForm({ ...form, email: e.target.value })} className="bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-xs col-span-2" />
          <input placeholder="Téléphone" onChange={(e) => setForm({ ...form, phone: e.target.value })} className="bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-xs" />
          <input placeholder="Entreprise" onChange={(e) => setForm({ ...form, company: e.target.value })} className="bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-xs" />
          <input placeholder="LinkedIn URL" onChange={(e) => setForm({ ...form, linkedinUrl: e.target.value })} className="bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-xs col-span-2" />
        </div>
